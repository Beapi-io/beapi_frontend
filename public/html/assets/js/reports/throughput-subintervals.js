	...
	function getLocations(eventName) {
	    console.log(window.token.access_token);
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/throughputSub/getArchiveLocationsByEvent?id="+eventName,
			crossDomain: true,
			cache:false,
			xhrFields: {
	...
	function getReportData(elem){
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/throughputSub/listByPacketReport?id="+elem.value,
			crossDomain: true,
			cache:false,
			xhrFields: {
				withCredentials: false
