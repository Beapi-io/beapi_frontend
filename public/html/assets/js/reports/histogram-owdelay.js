	...
	function getLocations(eventName) {
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/packetReport/getArchiveLocationsByEvent?id="+eventName,
			crossDomain: true,
			cache:false,
			xhrFields: {
			...
			headers:{
		  	...
		  	success: function(json) {
				...
				select.id='locations';
				// might be 'onchange'
				select.setAttribute("onchange", "getDates(this)");
	...
	function getDates(elem) {
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/owDelay/listByPacketReport?id="+elem.value,
			crossDomain: true,
			cache:false,
			xhrFields: {
				withCredentials: false
			...
			headers:{
		  	...
		  	success: function(json) {
				...
				$.each(json, function(key,val){
					etOption = document.createElement("option");
					etOption.text = timeConverter(val.timestamp);
					etOption.value = val.id;
					select.options.add(etOption);
				});
	...
	function getReportData(elem){
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/owDelay/getById?id="+elem.value,
			crossDomain: true,
			cache:false,
			xhrFields: {
				withCredentials: false
			...
			headers:{
		  	...
		  	success: function(json) {
				...
				var parentRow = document.getElementById("row3");
