	...
	function getLocations(eventName) {
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/packetRetransmit/getArchiveLocationsByEvent?id="+eventName,
			crossDomain: true,
			cache:false,
			xhrFields: {
	...
	function getReportData(elem){
		$.ajax({
			type:'GET',
			url: window.url+"/v0.1/packetRetransmit/listByPacketReport?id="+elem.value,
			crossDomain: true,
			cache:false,
			xhrFields: {
				withCredentials: false
			...
			headers:{
		  	...
		  	success: function(json) {
				...
				$.each(json, function(key,val){
					...
					obj.time = timeConverter(val.timestamp);
					obj.count = val.val;
